...
4. **Judge each statement.** Write `judgements.yaml`: for each statement (and any
   ...
   safety and commissioning impact. Weigh conflicting sources explicitly. An `aligned`
   verdict needs the same scrutiny as a change: say which source supports the specific
   value, or that none does. Where no guideline covers a point, still recommend on
